...
private:
        ...
        std::shared_ptr<const CountUntil::Goal> goal)
    {
        ...
        // Policy: reject goals if active goal exists
        // {
        ...
        //         {
        //             RCLCPP_INFO(this->get_logger(), "Rejected goal because another goal is active.");
        ...
        // Policy: Preempt existing goal when receiving a new valid goal.
         {
             std::lock_guard<std::mutex> lock(goal_handle_mutex_);
                if (goal_handle_)
                {
                    if (goal_handle_->is_active())
                    {
                        RCLCPP_INFO(this->get_logger(), "Abort Current Goal to Preempt with New Goal.");
                        preempted_goal_uuid_ = goal_handle_->get_goal_id();
                    }
                }
         }
        ...
        const std::shared_ptr<CountUntilGoalHandle> goal_handle)
    {
        ...
        for(int i = 0; (i < target_number);i++)
        {
            {
                std::lock_guard<std::mutex> lock(goal_handle_mutex_);
                if (goal_handle->get_goal_id()== preempted_goal_uuid_)
                {
    ...
    rclcpp_action::GoalUUID preempted_goal_uuid_;
};
