...
class TurtleSpawnerNode(Node):
    def __init__(self):
        ...
        self.turtle_name_prefix_ = self.get_parameter('turtle_name_prefix').get_parameter_value().string_value
        ...
        self.spawn_turtle_timer_ = self.create_timer(1.0/self.spawn_frequency_, self.spawn_turtle_callback)
    ...
    def spawn_turtle_callback(self):
        ...
        self.call_spawn_service(x, y, theta, turtle_name)
    ...
    def callback_call_spawn_service(self, future, request: Spawn.Request):
        ...
        if response.name != "":
            self.get_logger().info(' New turtle alive: ' + request.name)
            new_turtle = Turtle() 
