...
public:
    ...
    {
        this->declare_parameter<double>("spawn_frequency", 1.0);
        this->get_parameter("spawn_frequency", spawn_frequency_);
        this->declare_parameter<std::string>("turtle_name_prefix", "turtle_");
        this->get_parameter("turtle_name_prefix", turtle_name_prefix_);
        ...
        spawn_turtle_timer_ = this->create_wall_timer(
            std::chrono::duration<double>(1.0 / spawn_frequency_), std::bind(&TurtleSpawnerNode::spawnNewTurtle, this));
...
private:
    ...
    void callbackCallKillTurtleService(rclcpp::Client<turtlesim::srv::Kill>::SharedFuture future)
    {
        ...
        {
            if (alive_turtles_.at(i).name == turtle_to_catch_)
            {
